...
export function stations_datatype(db:D1Database) {
    ...
    async function batchUpsert(tuples:IDataSet[]): Promise<void> {
        ...
        const req = db.prepare(
            "INSERT INTO " +
            "stations_datatype (station_id, adcp, cwind, dart, drift, ocean, rain, spec, spectral, srad, supl, txt, last_updated)" +
            ...
            "WHERE " +
            "stations_datatype.adcp IS DISTINCT FROM excluded.adcp OR " +
            ...
            "stations_datatype.ocean IS DISTINCT FROM excluded.ocean OR " +
